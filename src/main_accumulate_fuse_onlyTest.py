...
def load_iters_lstm(args, batch_size=32, backward_batch_size=1000, device="cpu", gold_data_path='data', syn_data_path='data', vectors=None, use_tree=False, num_use_samples_inner=100, num_use_samples_outer=100, shuffle_train=True):
    ...
    for i in range(args.len_LLM):
        if args.steps == 0:
            ...
            train_data_path = f'{SYN_DATA_PATH}{args.gen_sample_select}/{args.model_name_sample_use_data}/{args.small_model_name}/{args.fuse_dataset_sample_selection}_KD{args.kd_slm}_FuseDataset{args.fuse_dataset}/fewshotK{args.gen_few_shot_k}_{args.gen_few_shot_pool_size}_{args.gen_few_shot_ambiguous_ratio}/{args.seed}/{args.llms[i]}/{args.num_use_samples_inner[i]}_{args.num_use_samples_init[i]}_{args.num_use_samples_each_step_extend[i]}/'
        train_data, _ = data.TabularDataset.splits(
            path=train_data_path,
            train='train.jsonl' if 'Flip' in args.fuse_dataset_sample_selection else 'train_noflip.jsonl',
            test='train.jsonl' if 'Flip' in args.fuse_dataset_sample_selection else 'train_noflip.jsonl',
            # train='test.jsonl',
            # test='test.jsonl',
            format='json',
            fields=fields,
...
def load_iters_bert(args, batch_size=32, backward_batch_size=1000, device="cpu", gold_data_path='data', syn_data_path='data', vectors=None, use_tree=False, num_use_samples_inner=100, num_use_samples_outer=100, shuffle_train=True):
    ...
    for i in range(args.len_LLM):
        ...
        if args.steps == 0:
            ...
            train_data_path = f'{SYN_DATA_PATH}{args.gen_sample_select}/{args.model_name_sample_use_data}/{args.small_model_name}/{args.fuse_dataset_sample_selection}_KD{args.kd_slm}_FuseDataset{args.fuse_dataset}/fewshotK{args.gen_few_shot_k}_{args.gen_few_shot_pool_size}_{args.gen_few_shot_ambiguous_ratio}/{args.seed}/{args.llms[i]}/{args.num_use_samples_inner[i]}_{args.num_use_samples_init[i]}_{args.num_use_samples_each_step_extend[i]}/' + ('train.jsonl' if 'Flip' in args.fuse_dataset_sample_selection else 'train_noflip.jsonl')
...
if __name__ == "__main__":
    ...
    assert args.task_name in ['sst2', 'yelp', 'mnli', 'mnliMisM'], f"[ERROR] task should within ['sst2', 'mnli', 'mnliMisM'], but current task is {args.task_name}"
    args.task_data_mapping = {
        'imdb': 'imdb',
        'sst2': 'imdb',
        'yelp': 'yelp',
        'mnli': 'mnliMisM',
    ...
    args.model_name_sample_use_data = f'{args.task_data_mapping[args.task_name]}/[mix]_{args.llms[0]}_{args.num_use_samples_inner[0]}' if args.mix else f'{args.task_data_mapping[args.task_name]}/{args.llms[0]}_{args.num_use_samples_inner[0]}'
    for _model, num_samples_inner in zip(args.llms[1:], args.num_use_samples_inner[1:]):
        ...
        args.model_name_sample_use_data += f'__{_model}_{num_samples_inner}'
    ...
    args.result_file_path = f'./results/eval_on_real/few_shot_accumulate/{args.small_model_name}/{args.train_ratio}_{args.weight_adjust_criterial}_{args.fuse_dataset_weight}_{args.fuse_dataset_sample_selection}_{args.kd_aggregate_weight}_KD{args.kd_slm}_FuseDataset{args.fuse_dataset}/{args.kd_alpha}_{args.kd_temperature}_init{args.num_use_samples_init[0]}_steps{args.steps}/fewshotK{args.gen_few_shot_k}_{args.gen_few_shot_pool_size}_{args.gen_few_shot_ambiguous_ratio}/{args.model_name_sample}/{args.seed}/'
    ...
    if args.steps == 0:
        ...
        for im in range(args.len_LLM):
            # args.init_sample_path.append(f'data_accumulate_start/{args.task_data_mapping[args.task_name]}/{args.llms[im]}/{args.num_use_samples_inner[im]}_{args.num_use_samples_init[im]}/train.jsonl')
            ...
            args.working_sample_dir.append(f'{SYN_DATA_PATH}{args.gen_sample_select}/{args.model_name_sample_use_data}/{args.small_model_name}/{args.fuse_dataset_sample_selection}_KD{args.kd_slm}_FuseDataset{args.fuse_dataset}/fewshotK{args.gen_few_shot_k}_{args.gen_few_shot_pool_size}_{args.gen_few_shot_ambiguous_ratio}/{args.seed}/{args.llms[im]}/{args.num_use_samples_inner[im]}_{args.num_use_samples_init[im]}_{args.num_use_samples_each_step_extend[im]}/')
        ...
        for i_step in range(args.steps, args.steps+1):
            args.i_step = i_step
